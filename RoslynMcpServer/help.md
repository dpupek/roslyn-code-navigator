...
- `GetTestStatus`: returns `TestRunStatusResult` with `Succeeded`, `Message`, and `Status` (`RunId`, `State`, `ExitCode`, `HasExited`, `LastLogTimestamp`, `FailedTests`, `FirstFailureMessage`, `StdOutTail`, `StdErrTail`, `LogFilePath`, `TrxPath`). If process/log diagnostics fail, `State` is set to `Inconclusive` and the tool still returns successfully.
