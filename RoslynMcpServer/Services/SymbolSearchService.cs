...
namespace RoslynMcpServer.Services
{
    public class SymbolSearchService
    {
        ...
        public async Task<IEnumerable<SymbolSearchResult>> SearchSymbolsAsync(
            string pattern, string solutionPath, string symbolTypes, bool ignoreCase, CancellationToken cancellationToken = default)
        {
            ...
            // Search across all projects with bounded parallelism
            var compilableProjects = solution.Projects.Where(p => p.SupportsCompilation).ToList();
            var throttledTasks = new List<Task<IEnumerable<SymbolSearchResult>>>(compilableProjects.Count);
            using var throttle = new SemaphoreSlim(_maxParallelProjectOperations);
            
            foreach (var project in compilableProjects)
            {
                var projectToSearch = project;
                ...
                throttledTasks.Add(Task.Run(async () =>
                {
                    try
                    {
                        return await SearchProjectSymbolsAsync(projectToSearch, regex, typeFilter, cancellationToken).ConfigureAwait(false);
                    }
                    finally
                    {
                        throttle.Release();
            ...
            var projectResults = await Task.WhenAll(throttledTasks).WaitAsync(cancellationToken);
        ...
        private async Task<IEnumerable<SymbolSearchResult>> SearchProjectSymbolsAsync(
            Project project, Regex pattern, HashSet<SymbolKind> typeFilter, CancellationToken cancellationToken)
        {
            var results = new List<SymbolSearchResult>();
        ...
        {
            var configured = Environment.GetEnvironmentVariable("ROSLYN_MAX_PROJECT_CONCURRENCY");
            ...
            var heuristic = Math.Clamp(logicalCores / 2, 1, 8);
            return heuristic;
        }
    }
}
